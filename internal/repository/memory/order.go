...
func (m *MemStorage) SaveOrders(ctx context.Context, number string, orders models.Order) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *MemStorage) GetOrders(ctx context.Context, login string) ([]models.Orders, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
